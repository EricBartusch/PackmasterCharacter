...
public class ExhaustRandomNonArtCardsAction extends AbstractGameAction {
    ...
    @Override
    public void update() {
        int validCardsCount = AbstractDungeon.player.hand.group.stream().filter(card -> !(card instanceof ArtCard)).collect(Collectors.toList()).size();
